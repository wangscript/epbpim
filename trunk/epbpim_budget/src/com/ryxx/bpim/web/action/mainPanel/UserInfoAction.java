...
public class UserInfoAction extends ActionSupportBase {
	...
	public String sendPasswordByEmail() {
		...
		if (userInfo == null) {
			...
			super.addFieldError("name", "您还没有填写过邮箱，无法发送。请联系您的企业账号管理员或联系平台客服人员");
		} else {
			...
			String emailContent = EmailTools.generateFindPasswordContent(
					userInfo.getRealName(), newPassword);
			List<String> addresses = new ArrayList<String>();
			addresses.add(userInfo.getEmail());
			try {
				EmailTools.send(addresses, Constants.EMAIL_SUBJECT,
						emailContent);
